...
public static class EditotGUITools
{
    private static readonly Texture2D backgroundTexture2D = Texture2D.whiteTexture;

    private static readonly GUIStyle textureStyle = new GUIStyle
        { normal = new GUIStyleState { background = backgroundTexture2D } };

    public static void DrawRect(Rect pos,Color color,GUIContent content = null)
    {
        color.a = 0.5f;
        var backgroundColor = GUI.backgroundColor;
        GUI.backgroundColor = color;
        GUI.Box(pos,content?? GUIContent.none,textureStyle);
